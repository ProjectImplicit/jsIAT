...
(function($) {
    ...
    $.extend ({
        xmlToJSON: function(xdoc) {
            try {
                var err = xdoc.documentElement;
                if(err.tagName == 'parsererror'){
                    var err_message = {};
                    err_message.error = err.childNodes[0].nodeValue;
                    return err_message;
                }
                if(!xdoc){
                    return null;
                        ...
                        do { //Order is irrelevant (speed-up)
                            ...
                            switch(cnode.nodeType) {
                                ...
                                case 3: //Text Value
                                    ...
                                    break;
                                case 4: //CDATA
                                    obj.Text += replaceFont((cnode.text)?$.trim(cnode.text):$.trim(cnode.nodeValue));
                                    break;
                            }
                        } while(n++ < nodeCount);
                    }
